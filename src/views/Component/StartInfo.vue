...
info.value = Object.assign(state.value, {
  playerRole: Math.floor(Math.random()*10*20%2) == 0 ? 'emperor' : 'slave',
  rounds: 1,
})
// 初始化手牌
...
<template>
  <div v-if="showGameInfo" flex-center flex-col h-screen w-screen relative font-size-40px bg-gray:50>
    <div w-160px h-56px flex-center rd-8px gap-5>第 <span text-green> {{ info?.rounds }} </span> 局</div>
    <div w-240px h-56px flex-center rd-8px>角色 ： <span text-red>{{ name }}</span> </div>
  </div>
</template>
